<template>
  <nav class="border-green-400 bg-green-400">
          ...
          <li v-for="item in navItems" :key="item.name">
            <router-link :to="item.path"
              :class="[
                ...
                item.active
                  ? 'text-white bg-blue-700 md:bg-transparent md:text-blue-700 md:dark:text-blue-500 dark:bg-blue-600'
...
export default {
  data() {
    return {
      navItems: [{ name: "Home", path: "/" }, { name: "Cart", path: "/cart" }],
    };
  },
};
</script>
